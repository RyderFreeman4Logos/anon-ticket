...
afa32f1 feat(api): embed monitor for bloom prewarm
Exposed the monitor as a library (with RPC builder and hooks) and made the API bootstrap spin an embedded monitor when config is present, sharing cache/Bloom via `MonitorHooks`. Added a boot-time prewarm that loads all payment IDs from storage into cache/Bloom, with logging to track elapsed time, plus a dev-only `API_ALLOW_NO_MONITOR` bypass so tests can run without Monero RPC. Standalone monitor binaries now require `ALLOW_STANDALONE_MONITOR=1` to prevent accidental production split-brain deployments. `SeaOrmStorage` gained `all_payment_ids()` to fuel prewarming, and new join/error plumbing ensures monitor failures bubble up instead of silently dying.
28137f4 feat(api): require bloom unless dev override
Hardened startup so the PID Bloom filter cannot be disabled in production: if `API_PID_BLOOM_ENTRIES=0` the API now fails to boot unless `API_ALLOW_NO_BLOOM` is explicitly set. Bloom parameters are logged at boot, and defaults still apply for unset env vars. This closes the gap where a missing Bloom would remove the primary DoS shield in Tor deployments while keeping a dev/test escape hatch for environments without sizing tuned yet.
98620d5 feat(api): switch to bloom-only redemption
Removed the negative PID cache and its grace-window/env plumbing; redemption now gates solely on the Bloom filter (404 on Bloom negative), then proceeds to storage. Absent PIDs no longer write into Bloom, while confirmed records still mark Bloom/cache present. AppState dropped the negative-grace field, ApiConfig no longer parses `API_PID_CACHE_NEGATIVE_GRACE_MS`, and tests were rewritten to cover Bloom short-circuit vs. success/absent cases.
5a5b432 chore(api): log bloom sizing and fp misses
Added derived Bloom sizing telemetry (estimated bytes from entries+FPR) to startup logs and introduced `api_redeem_bloom_db_miss_total` to track Bloom-positive/DB-miss false positives. Bloom cache hints now distinguish positive vs. absent, keeping FP monitoring visible alongside the new Bloom-only redemption path.
949c836 docs(todo): mark bloom hardening milestones done
Checked off ShortTerm-34..38 in the roadmap to reflect the embedded monitor, mandatory Bloom, Bloom-only redemption, added telemetry, and doc updates describing the Tor/No-IP DoS posture and dev escape hatches.
a2824f8 docs(api): describe bloom-only defense and config
Updated root/API/domain docs to reflect the Bloom-only redemption model: Bloom negatives 404 immediately, cache is positive-only, negative-grace env is removed, Bloom disablement requires `API_ALLOW_NO_BLOOM`, and metrics now surface Bloom hints and FP misses. Config tables and design notes were simplified to match the new posture.
